    ...
    <x-slot name="content">
        ...
        @if ($this->enabled)
            @if ($showingQrCode)
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold">
                        @if ($showingConfirmation)
                            {{ __("Pour terminer l'activation de l'authentification à deux facteurs, scannez le QR code suivant avec l'application d'authentification de votre téléphone ou entrez la clé de configuration et fournissez le code OTP généré.") }}
                        @else
                            {{ __("L'authentification à deux facteurs est maintenant activée. Scannez le QR code suivant avec l'application d'authentification de votre téléphone ou entrez la clé de configuration.") }}
                        @endif
                    </p>
                </div>
            ...
            @if ($showingRecoveryCodes)
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold">
                        {{ __("Enregistrez ces codes de récupération dans un gestionnaire de mots de passe sécurisé. Ils peuvent être utilisés pour récupérer l'accès à votre compte si votre dispositif d'authentification à deux facteurs est perdu.") }}
                    </p>
                </div>
